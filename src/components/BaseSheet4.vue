    ...
    <q-list id="stat-block">
      ...
      <q-expansion-item v-if="character.defense" id="defense"
        ...
        <q-item class="">
          ...
          <q-item-section class="health justify-around rounded-borders">
            <div class="q-gutter-xl row no-wrap justify-center items-center">
              <q-btn
                @click="damageTaken = 0"
                label="Full Heal"
                size="lg"/>
              ...
              <q-btn
                ...
                label="-"/>
              <q-input class="content-center"
                       style="color: black; font-size: xx-large"
      ...
      <q-expansion-item v-if="character.statistics" id="statistics"
        ...
        <div id="ability scores">
          <span v-for="(score, key, index) in character.abilityScores.value"
                v-bind:key="index">
            <b class="capitalize"> {{ key.substring(0, 3) }}</b> {{ score }}
            ({{
              formatBonus(
...
.health {
  background-color: rgba(190, 132, 87, 0.9);
}
